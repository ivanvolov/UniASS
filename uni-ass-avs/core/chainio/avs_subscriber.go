...
type AvsSubscriberer interface {
	SubscribeToNewTasks(newTaskCreatedChan chan *cstaskmanager.ContractUniASSTaskManagerNewTaskCreated) event.Subscription
	SubscribeToTaskResponses(taskResponseLogs chan *cstaskmanager.ContractUniASSTaskManagerTaskResponded) event.Subscription
	ParseTaskResponded(rawLog types.Log) (*cstaskmanager.ContractUniASSTaskManagerTaskResponded, error)
}
...
func BuildAvsSubscriberFromConfig(config *config.Config) (*AvsSubscriber, error) {
	return BuildAvsSubscriber(
		config.UniASSRegistryCoordinatorAddr,
...
func (s *AvsSubscriber) SubscribeToNewTasks(newTaskCreatedChan chan *cstaskmanager.ContractUniASSTaskManagerNewTaskCreated) event.Subscription {
...
func (s *AvsSubscriber) SubscribeToTaskResponses(taskResponseChan chan *cstaskmanager.ContractUniASSTaskManagerTaskResponded) event.Subscription {
...
func (s *AvsSubscriber) ParseTaskResponded(rawLog types.Log) (*cstaskmanager.ContractUniASSTaskManagerTaskResponded, error) {
	return s.AvsContractBindings.TaskManager.ContractUniASSTaskManagerFilterer.ParseTaskResponded(rawLog)
}
